...
interface CheckoutReviewProps {
  address: Address;
  items: any[];
  handlePlaceOrder: (paymentMethodOverride?: string) => void;
  loading: boolean;
  total: number;
}
...
export default function CheckoutReview({
  ...
  return (
    <div className="bg-white rounded-2xl p-6 animate-fade-in">
      ...
      {import.meta.env.DEV && (
        <button
          onClick={() => handlePlaceOrder("dev_bypass")}
          disabled={loading}
          className="w-full mt-3 py-3 bg-zinc-800 text-white font-semibold rounded-xl hover:bg-zinc-900 transition-colors disabled:opacity-60 active:scale-[0.98]"
        >
          {loading ? "Processing..." : `Bypass Payment (Dev Only)`}
        </button>
      )}
    </div>
  );
